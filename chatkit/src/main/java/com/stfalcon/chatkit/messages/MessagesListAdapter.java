...
public class MessagesListAdapter<MESSAGE extends IMessage>
     ...
     * For default list item layout and view holder.
     *
     * @param selfId    identifier of sender.
     * @param holders     custom layouts and view holders. See {@link MessageHolders} documentation for details
     * @param imageLoader image loading method.
     */
    public MessagesListAdapter(String selfId, MessageHolders holders,
    ...
    public int getItemViewType(int position) {
        return holders.getViewType(items.get(position).item, selfId);
    }
    ...
    private boolean isPreviousSameAuthor(String id, int position) {
        ...
        if (items.size() <= prevPosition) return false;
                ...
                && ((MESSAGE) items.get(prevPosition).item).getAuthorId().contentEquals(id);
    }
	...
	private void notifyBindPosition(int pos) {
		if (onBindViewHolder != null) {
			onBindViewHolder.OnBindPosition(pos);
		}
	}
