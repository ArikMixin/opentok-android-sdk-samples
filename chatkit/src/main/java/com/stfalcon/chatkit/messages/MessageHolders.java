...
public class MessageHolders {
    ...
    protected int getViewType(Object item, String selfId) {
        boolean isOutcoming = false;
        int viewType;

        if (item instanceof IMessage) {
            IMessage message = (IMessage) item;
            isOutcoming = message.getAuthorId().contentEquals(selfId);
...
//                        && message.getContact().getAvatar() != null
//                        && !message.getContact().getAvatar().isEmpty();
//
//                userAvatar.setVisibility(isAvatarExists ? View.VISIBLE : View.GONE);
//                if (isAvatarExists) {
//                    imageLoader.loadImage(userAvatar, message.getContact().getAvatar(), null);
//                }
//            }
        }
