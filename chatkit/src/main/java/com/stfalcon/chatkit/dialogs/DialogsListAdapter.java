    ...
    public boolean updateDialogWithMessage(String dialogId, IMessage message) {
        ...
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getId().equals(dialogId)) {
                items.get(i).setLastMessageCreatedDate(message.getCreatedAt());
                items.get(i).setLastMessageText(message.getText());
                notifyItemChanged(i);
                if (i != 0) {
    ...
    public void sortByLastMessageDate() {
        Collections.sort(items, new Comparator<DIALOG>() {
            ...
            public int compare(DIALOG o1, DIALOG o2) {
                if (o1.getLastMessageCreatedDate().after(o2.getLastMessageCreatedDate())) {
                    return -1;
                } else if (o1.getLastMessageCreatedDate().before(o2.getLastMessageCreatedDate())) {
                    return 1;
                } else return 0;
            }
        });
...
//                imageLoader.loadImage(ivLastMessageUser, dialog.getLastMessage().getContact().getAvatar(), null);
//            }
...
//                    && ((dialog.getLastMessageId() != null)&&(!dialog.getLastMessageId().equals(""))) ? VISIBLE : GONE);
            ...
            if ((dialog.getLastMessageId() != null)&&(!dialog.getLastMessageId().equals(""))) {
                tvLastMessage.setText(dialog.getLastMessageText());
            } else {
