...
public abstract class WCDatabase2 extends RoomDatabase {

	public abstract UserDao userDao();

	// marking the instance as volatile to ensure atomic access to the variable
	private static volatile WCDatabase2 INSTANCE;

	static WCDatabase2 getDatabase(final Context context) {
		if (INSTANCE == null) {
			synchronized (WCDatabase2.class) {
				if (INSTANCE == null) {
					INSTANCE = Room.databaseBuilder(context.getApplicationContext(),
						WCDatabase2.class, "user_database")
	...
	private static class PopulateDbAsync extends AsyncTask<Void, Void, Void> {
		...
		PopulateDbAsync(WCDatabase2 db) {
			mDao = db.userDao();
		}
